...
class ComplementarityEstimationModule(nn.Module):
    def __init__(self, in_channels=128, use_multiplication=False):
        ...
        # 3-part descriptor dim = 128 * 3 = 384
        # 4-part descriptor dim = 128 * 4 = 512
    ...
    def forward(self, Fr, Ft):
        zr = self.gap(Fr).squeeze(-1).squeeze(-1)  # [B, 128]
        zt = self.gap(Ft).squeeze(-1).squeeze(-1)  # [B, 128]
        
        if self.use_multiplication:
            # Dc = [zr, zt, |zr - zt|, zr * zt] -> dim 512
            Dc = torch.cat([zr, zt, torch.abs(zr - zt), zr * zt], dim=-1)
        else:
            # Dc = [zr, zt, |zr - zt|] -> dim 384 (Default starting point)
            Dc = torch.cat([zr, zt, torch.abs(zr - zt)], dim=-1)
            
        Sc = self.mlp(Dc)  # Output channel-wise weight: [B, 128]
        return Sc