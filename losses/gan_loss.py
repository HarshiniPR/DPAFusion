...
class SobelGrad(nn.Module):
    ...
    def forward(self, img):
        # Convert RGB to grayscale for gradient if 3 channels
        if img.shape[1] == 3:
...
class Stage4LossSuite(nn.Module):
    """
    Full Generator and Dual Discriminator Loss Functions:
    L_G = L_adv + lambda_grad*L_grad + lambda_pixel*L_pixel
    """
    def __init__(self, lambda_grad=20.0, lambda_pixel=5.0):
        super().__init__()
    ...
    def g_adversarial_loss(self, d_fake_rgb, d_fake_th):
        # Generator fools both D_rgb and D_th
        real_labels_rgb = torch.ones_like(d_fake_rgb)
        real_labels_th = torch.ones_like(d_fake_th)
        return self.bce(d_fake_rgb, real_labels_rgb) + self.bce(d_fake_th, real_labels_th)

    def g_content_loss(self, I_fused, Ir, It):
        # Thermal repeated across channels for pixel comparison
        if It.shape[1] == 1:
            It_3c = It.repeat(1, 3, 1, 1)
        else:
            It_3c = It

        # 1. Pixel/Intensity Preservation (Max rule across sources)
