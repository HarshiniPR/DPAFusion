...
class FiLMBlock(nn.Module):
    ...
    def forward(self, x, d_rec):
        ...
        gamma = torch.tanh(gamma).unsqueeze(-1).unsqueeze(-1)  # Bound scaling factor
        beta = torch.tanh(beta).unsqueeze(-1).unsqueeze(-1)    # Bound shifting factor
...
class DCLARNetGenerator(nn.Module):
    def __init__(self, in_channels=128, cond_dim=2):
        ...
        # 1. Feature normalization and bottleneck
        ...
        # 6. Reconstruction head using Tanh
        self.head = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=3, padding=1, bias=False),
            nn.InstanceNorm2d(16, affine=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(16, 1, kernel_size=1),
            nn.Tanh()  # Tanh avoids sigmoid saturation
        )
    ...
    def forward(self, F_fused, actions):
        ...
        # Normalize incoming high-dynamic-range features
        ...
        # Output scaled cleanly to [0, 1] without sigmoid clipping
        I_out = self.head(x)
